...
// const clearDepartureCities = async () => {
//   try {
//     await AsyncStorage.removeItem('departureCities');
//     console.log('✅ departureCities removido!');
//   } catch (error) {
//     console.error('Erro ao remover departureCities:', error);
//   }
// };
...
>([]);
const { updateOrigin } = useSearchContext();
  ...
  try {
    ...
    if (cities !== null) {
      ...
      if (Array.isArray(parsed)) {
        setPreviousSelectedDeparture(parsed);
      }
    }
  } catch (error) {
  ...
  const autoCompleteSearch = async (searchInput: string) => {
    ...
    try {
      ...
      if (error.response && error.response.status === 429) {
        // Exibir uma mensagem de erro na tela ou desativar a busca temporariamente
        console.error(
          "Limite de taxa Amadeus excedido. Tente novamente em um minuto."
        );
        // Você pode adicionar um estado aqui para mostrar uma mensagem de erro na UI
      } else {
        console.error(error);
      }
    }
  ...
  const handleSelectAutoComplete = async (item: CityItem) => {
    ...
    updateOrigin(newCity.city, newCity.iataCode);

    const updatedCities = [...previousSelectedDeparture, newCity];
  ...
  return (
    <View className="flex-1 items-center bg-[#f5f7fa]">
      <View className="w-full h-full">
        <View className="justify-start border-orange-600 w-full bg-[#9BB5E9] relative rounded-b-[25px] px-6 pt-16 pb-8">
          <View>
            {/* Header */}
          ...
          <View className="px-3 w-full">
            ...
            {previousSelectedDeparture.map((city, index) => (
              <Pressable
                key={index}
                onPress={() => {
                  updateOrigin(city.city, city.iataCode);
